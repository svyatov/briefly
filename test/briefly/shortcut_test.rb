...
class ShortcutTest < BrieflyTest
  def test_defines_real_methods_for_canonical_and_aliases
    facade = Briefly.new { shortcut(:config, :c, :cfg) { :value } }
  ...
  def test_forwards_positional_arguments
    facade = Briefly.new { shortcut(:echo) { |*args| args } }

    assert_equal [1, 2], facade.echo(1, 2)
  end

  def test_forwards_keyword_arguments
    facade = Briefly.new { shortcut(:echo) { |**kwargs| kwargs } }
  ...
  def test_forwards_the_callers_block
    facade = Briefly.new { shortcut(:with_block) { |*args, &blk| [args, blk&.call] } }
  ...
  def test_body_arity_is_strict
    facade = Briefly.new { shortcut(:none) { :ok } }
  ...
  def test_redefinition_drops_stale_aliases
    facade = Briefly.new { shortcut(:config, :c) { :first } }
  ...
  def test_reserved_canonical_name_raises
    error = assert_raises(Briefly::ReservedNameError) { Briefly.new { shortcut(:class) { 1 } } }

    assert_match(/class is reserved/, error.message)
  end

  def test_reserved_alias_raises
    assert_raises(Briefly::ReservedNameError) { Briefly.new { shortcut(:fine, :send) { 1 } } }
  end

  def test_reserved_lifecycle_names_raise
    %i[shortcuts shortcut? clear_memos! reset! configure inspect to_s].each do |name|
      assert_raises(Briefly::ReservedNameError) { Briefly.new { shortcut(name) { 1 } } }
    end
  end
  ...
  def test_raise_is_reserved
    assert_raises(Briefly::ReservedNameError) { Briefly.new { shortcut(:raise) { :nope } } }
  end
  ...
  def test_shortcut_requires_a_block
    assert_raises(ArgumentError) { Briefly.new { shortcut(:x) } }
  end

  def test_shortcut_names_must_be_symbols
    assert_raises(ArgumentError) { Briefly.new { shortcut("x") { 1 } } }
  end

  def test_shortcut_returns_the_canonical_name
    name = nil
    Briefly.new { name = shortcut(:x, :y) { 1 } }
